 ...
 *
    ...
    console.log(hangman.draw()); // returns -1
 */
let hangman = new HangmanSvg(document.getElementById("hangman"));
...
function setUp() {
  ...
  displayLetters.innerHTML = `${words[count % total]
    .split("")
    .map((letter) => {
      return `<span data-ns-test="letter"></span>`;
    })
    .join("")}`;
}
setUp();
...
function display(e) {
  if (words[count % total].includes(e)) {
    let displayLetters = document.getElementById("displayLetters");
    displayLetters.innerHTML = `${words[count % total]
      .split("")
      .map((letter) => {
        return `<span data-ns-test="letter">${
          correctLetters.includes(letter) ? letter : ""
        }</span>`;
      })
      .join("")}`;
    if (
      displayLetters.innerText.replace(/[\n]/g, "") === words[count % total]
    ) {
      winMessage();
    }
  } else {
    ...
    wrongLetters.push(e);
    let wrongLettersContainer = document.getElementById(
      "wrongLettersContainer"
    );
    wrongLettersContainer.innerHTML = `<p>Wrong</p>${wrongLetters.map(
      (letter) => {
        return `<span data-ns-test="wrong-letter">${letter}</span>`;
      }
    )}`;
  }
}
...
function winMessage() {
  ...
  document.getElementById("btn-container").classList.remove("hide");
  document.getElementById("result").innerText = "Congratulations! You Won!";
  document.getElementById("para1").innerText = null;
}

function loseMessage() {
  window.removeEventListener("keydown", pressEvent);
  document.getElementById("btn-container").classList.remove("hide");
  document.getElementById("result").innerText = "Unfortunately you lost.";
  document.getElementById("para1").innerText = `...the word was: ${
    words[count % total]
  }`;
}
function playAgain() {
  ...
  setUp();
  document.getElementById("btn-container").classList.add("hide");
  document.getElementById("wrongLettersContainer").innerHTML = null;
}